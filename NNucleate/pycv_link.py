...
def write_pycv_link(model, n_hid, n_layers, n_at, box_l, rc, fname, normalise=True):
    """Function that writes an input file for the coupling with plumed, based on a model that is passed in the parameters.
    This version of the input file will be faster but requires a Cython package "neighborlist" with the same signature as the one provided in the github repository. 
    ...
    :type box_l: float
    :param rc: Cut off radius for the neighbourlist generation
    :type rc: float
    :param fname: Name of file that is created.
    :type fname: str
    :param normalise: Whether to normalise the gradients, defaults to True
    ...
    with open(fname, "w") as f:
        ...
        f.writelines(
            [
                ...
                "import math\n",
                "import jax\n",
                "from neighborlist import compute_neighborlist\n",
        ...
        f.writelines(
            [
                "\n\ndef get_rc_edges(traj):\n",
                "    row_list = []\n",
                "    col_list = []\n",
                "    frame_row = []\n",
                "    frame_col = []\n",
                "\n",
                "    dist = compute_neighborlist(traj, %f, %f, %d)\n" % (rc, box_l, n_at),
                "\n",
                "    for j in range(%d):\n" % n_at,
        ...
        f.writelines(
            [
                ...
                "\n",
                "    x = x/10.0\n",
                "    x = pbc(x).astype(\"float32\")\n",
                "    rows, cols = get_rc_edges(x)\n",
                "\n",
                "    if len(rows[0]) == 0:\n",
...
def write_fast_link_2D(model, n_hid, n_layers, out_nodes, n_at, box_l, rc, dim1, dim2, fname, pool="sum", normalise=True):
    """Function that writes an input file for the coupling with plumed, based on a model that is passed in the parameters.
    This version of the input file will be faster but requires a Cython package "neighborlist" with the same signature as the one provided in the github repository. 
    This function assumes the following architecture:
    - Embedding layer n_at x n_hid
    ...
    :param n_layers: Number of GCL layers.
    :type n_layers: int
    :param out_nodes: Dimensionality of the model output.
    :type out_nodes: int
    :param n_at: Number of nodes in the graph.
    :type n_at: int
    :param box_l: Size of the simulation box of the system that is used in the MTD simulation.
    ...
    :param rc: Cut off radius for the neighbourlist generation
    :type rc: float
    :param dim1: Name of the CV in the first dimension.
    :type dim1: str
    :param dm2: Name of the CV in the second dimension.
    :type dim2: str
    :param fname: Name of file that is created.
    :type fname: str
    :param pool: Name of the pooling layer that is used. ("sum", "mean", "max")
    :type pool: str
    :param normalise: Whether to normalise the gradients, defaults to True
    :type normalise: bool, optional
    """
    with open(fname, "w") as f:
        ...
        f.write("    return {'%s': j_pred[0][0], '%s': j_pred[0][1]}\n" % (dim1, dim2))

        # jax grad
        f.write("\ngrad_mod = jacrev(jax_model)\n")
        ...
        f.writelines(
            [
                ...
                "\n",
                "    if len(rows[0]) == 0:\n",
                "        return {'%s': 0.0, '%s': 0.0}, {'%s': np.zeros(x.shape), '%s': np.zeros(x.shape)}\n" % (dim1, dim2, dim1, dim2),
                "\n",
                "    x = jnp.array(x/%f)\n" % box_l,
                "    pred = jax_model(x, rows[0], cols[0])\n",
                "    gs = grad_mod(x, rows[0], cols[0])\n",
                ...
                "    # Set the gradient of unconnected nodes 0 \n",
                "    inds = np.arange(0, len(x)) \n",
                "    dele = np.unique(rows[0])\n",
                "    gs = {'%s': np.array(gs['%s']), '%s': np.array(gs['%s'])}\n" % (dim1, dim1, dim2, dim2),
                "    gs['%s'][np.delete(inds, dele)] = 0.0\n" % dim1,
                "    gs['%s'][np.delete(inds, dele)] = 0.0\n" % dim2,])

        if normalise:
            f.writelines(
                [
                    "gs['%s'] = gs['%s']/np.linalg.norm(gs['%s'])\n" % (dim1, dim1, dim1),
                    "gs['%s'] = gs['%s']/np.linalg.norm(gs['%s'])\n" % (dim2, dim2, dim2),
                ]
            )
