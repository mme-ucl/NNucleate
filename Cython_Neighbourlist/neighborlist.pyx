...
def compute_neighborlist(traj, cutoff, box_length, n_at):
    ...
    cdef float[:, ::1] xyz = traj
    cdef float[:, ::1] box_matrix
    vecs = np.zeros((3, 3))
    vecs[0] = np.array([box_length, 0, 0]) 
    vecs[1] = np.array([0, box_length, 0]) 
    vecs[2] = np.array([0, 0, box_length])
    box_matrix = np.asarray(vecs, order='c', dtype=np.float32)
    cdef float* box_matrix_pointer = &box_matrix[0,0]
...
#    cdef int n_atoms = traj.xyz.shape[1]
    cdef vector[vector[int]] atom_neighbors = _compute_neighborlist(&xyz[0,0], n_at, cutoff, box_matrix_pointer)
