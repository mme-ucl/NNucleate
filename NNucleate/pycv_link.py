...
def write_cv_link(model, n_hid, n_layers, n_at, box_l, rc, fname):
    ...
    with open(fname, "w") as f:
        ...
        f.writelines(
            [
                ...
                "    rows, cols = get_rc_edges(%f, fake_traj[0])\n" % rc,
                "\n",
                "    if len(rows[0]) == 0:\n",
...
def write_fast_link(model, n_hid, n_layers, n_at, box_l, rc, fname):
    """Function that writes an input file for the coupling with plumed, based on a model that is passed in the parameters.
...
def write_fast_link_2D(model, n_hid, n_layers, out_nodes, n_at, box_l, rc, dim1, dim2, fname, pool="sum"):
    """Function that writes an input file for the coupling with plumed, based on a model that is passed in the parameters.
    This version of the input file will be faster but requires a Cython package "neighborlist" with the same signature as the one provided in the github repository. 
    This function assumes the following architecture:
    - Embedding layer n_at x n_hid
